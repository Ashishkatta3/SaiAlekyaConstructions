...
```
sai-alekya-website/
├── index.html          ← page structure only (content + section markup)
├── css/
│   └── styles.css      ← all styling
├── js/
│   └── main.js         ← all behaviour (menu, scroll, enquiry form)
├── images/             ← all photos and the floor plan
├── brochures/          ← the four project PDFs
├── favicon files / site.webmanifest
└── README.md           ← this file
```

The three parts are linked together automatically: `index.html` pulls in
`css/styles.css` in its `<head>` and `js/main.js` just before `</body>`, so you
never edit style or behaviour inside the HTML again — each lives in one place.

**Where to change things**
- Text, sections, images, links  →  `index.html`
- Colours, fonts, spacing, layout →  `css/styles.css`
- Menu, animations, enquiry form  →  `js/main.js`
...
### Receiving enquiries by email
The enquiry form sends to **both WhatsApp and email**. To turn on email:
1. Open `js/main.js` and set `ENQUIRY_EMAIL` (top of section 5) to your address.
2. Also update the `"email"` line in the schema block inside `index.html`.
3. The **first** time someone emails through the form, FormSubmit.co sends a
   one-time confirmation to that inbox — click the link once, and every enquiry
   after that arrives automatically as a tidy table. No signup, no server, free.
Visitors still get the WhatsApp button too, so nothing is lost if they prefer it.

### Before you go live
Search `index.html` for **YOURDOMAIN.in** and replace every
occurrence with your real domain (used by the SEO tags and the LocalBusiness
data Google reads). Also set your email in the two places noted above.
